...
(function() {
    ...
    const initUI = () => {
        ...
        if (langSelect) langSelect.addEventListener('change', populate);
        if (levelSelect) levelSelect.addEventListener('change', populate);
        ...
        if (hintBtn) hintBtn.onclick = () => {
            ...
            if (cells.length > 0) {
                // Hint economy deduction (Stage 5.3)
                if (typeof COSYGame !== 'undefined') {
                    COSYGame.addScore(-5);
                }
                ...
                if (window.gameUtils?.showGameMessage) {
                    window.gameUtils.showGameMessage(gameArea, "Hint economy used: -5 XP spent!", 'info');
                }
            }
        };
