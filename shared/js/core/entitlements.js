 ...
 *
 * Decentralized Cryptographic Gateway for COSYlanguages.
 * Consolidates passcodes and offline license keys under a namespaced key scheme:
 * COSY-{PRODUCT}-{SUBPRODUCT}-{TIER}
 *
 * Implements 100% client-side privacy-sovereign authentication and authorization.
 */
...
(function() {
    ...
    // NEW Core Namespaced Entitlements API
    window.COSY = window.COSY || {};
    window.COSY.entitlements = {
        ...
        check(productKey, subKey, tier) {
            ...
            const normalizedProduct = (productKey || '').toUpperCase();
            let normalizedSub = (subKey || '').toUpperCase();
            let normalizedTier = (tier || '').toUpperCase();

            // Support 'MINI' -> 'MINIGROUP' normalization
            if (normalizedTier === 'MINI') normalizedTier = 'MINIGROUP';

            const storageKey = `COSY-${normalizedProduct}-${normalizedSub}-${normalizedTier}`;

            // Check new namespaced storage
            if (sessionStorage.getItem(storageKey) === "true" || localStorage.getItem(storageKey) === "true") {
                return true;
            }

            // Backward Compatibility: check legacy storage keys for EVENT subproducts
            if (normalizedProduct === 'EVENTS') {
                const legacyTier = normalizedTier === 'MINIGROUP' ? 'mini' : (normalizedTier === 'PRIVATE' ? 'private' : normalizedTier.toLowerCase());

                if (normalizedSub.startsWith('WONDER-')) {
                    const draftKey = normalizedSub.split('-')[1];
                    const legacyKey = `cosy_wonder_auth_draft_${draftKey}_${legacyTier}`;
                    if (sessionStorage.getItem(legacyKey) === "true" || localStorage.getItem(legacyKey) === "true") {
                        return true;
                    }
                } else if (normalizedSub.startsWith('KUS-')) {
                    // Extract specimen ID from 'KUS-KUS01' or 'KUS-01'
                    let kusKey = normalizedSub.split('-')[1];
                    if (kusKey && !kusKey.startsWith('KUS')) {
                        kusKey = 'KUS' + kusKey;
                    }
                    const legacyKey = `cosy_science_auth_${kusKey}_${legacyTier}`;
                    if (sessionStorage.getItem(legacyKey) === "true" || localStorage.getItem(legacyKey) === "true") {
                        return true;
                    }
                }
        ...
        async unlock(rawCode) {
            ...
            // Support single digit padding (e.g. ICHBWMG4 -> ICHBWMG04)
            normalizedInput = normalizedInput.replace(/^(ICHBWMG|ICHBWPL|KUSMG|KUSPL)([1-9])$/, '$10$2');

            // 1. License Keys Flow: COSY-{PRODUCT}-{SUBPRODUCT}-{TIER} or checksum-based licenses
            if (normalizedInput.startsWith("COSY-")) {
                const segments = normalizedInput.split("-");
                if (segments.length >= 4) {
                    const product = segments[1];
                    const tier = segments[segments.length - 1];
                    const subproduct = segments.slice(2, segments.length - 1).join("-");

                    // CRITICAL SECURITY FIX: Prevent bypass of passcode-protected resources (Wonder and KUS)
                    // If the user enters a storage key pattern like COSY-EVENTS-WONDER-04-MINIGROUP directly as the passcode input,
                    // we MUST block it from unlocking unless it went through the cryptographic match.
                    if (product === 'EVENTS' && (subproduct.startsWith('WONDER-') || subproduct.startsWith('KUS-'))) {
                        return { success: false };
                    }

                    const storageKey = `COSY-${product}-${subproduct}-${tier}`;
                    localStorage.setItem(storageKey, "true");
                    sessionStorage.setItem(storageKey, "true");
                    return { success: true, productKey: product, subproduct, tier, isLicense: true };
                }
            }

            // 2. Wonder Club Passcodes Flow
            const wonderMatch = wonderMatchCheck(normalizedInput);
            if (wonderMatch) {
                const prefix = wonderMatch[1];
                const draftKey = wonderMatch[2];
                const tier = prefix === "ICHBWMG" ? "MINIGROUP" : "PRIVATE";
                const mode = prefix === "ICHBWMG" ? "mini" : "private";

                // Enforce context safety if current draft is present on page
                if (window.COSY_PASSCODES && typeof window.COSY_PASSCODES.getCurrentDraftKey === 'function') {
                    const currentDraft = window.COSY_PASSCODES.getCurrentDraftKey();
                    if (currentDraft && currentDraft !== draftKey) {
                        return { success: false };
                    }
                }

                const targetHash = HASH_DB[draftKey]?.[mode];
                if (targetHash) {
                    const computedHash = await sha256(normalizedInput);
                    if (computedHash === targetHash) {
                        const storageKey = `COSY-EVENTS-WONDER-${draftKey}-${tier}`;
                        ...
                        return { success: true, productKey: "EVENTS", subproduct: `WONDER-${draftKey}`, tier };
                    }
                }
            }

            // 3. KUS Passcodes Flow
            const kusMatch = normalizedInput.match(/^(KUSMG|KUSPL)([0-9]{2})$/);
            if (kusMatch) {
                const prefix = kusMatch[1];
                const specimenNum = kusMatch[2];
                const kusKey = "KUS" + specimenNum;
                const tier = prefix === "KUSMG" ? "MINIGROUP" : "PRIVATE";
                const mode = prefix === "KUSMG" ? "mini" : "private";

                // Enforce context safety if current specimen is present on page
                if (window.COSY_PASSCODES && typeof window.COSY_PASSCODES.getCurrentKusKey === 'function') {
                    const currentKus = window.COSY_PASSCODES.getCurrentKusKey();
                    if (currentKus && currentKus !== kusKey) {
                        return { success: false };
                    }
                }

                const targetHash = HASH_DB[kusKey]?.[mode];
                if (targetHash) {
                    const computedHash = await sha256(normalizedInput);
                    if (computedHash === targetHash) {
                        const storageKey = `COSY-EVENTS-KUS-${kusKey}-${tier}`;
                        ...
                        return { success: true, productKey: "EVENTS", subproduct: `KUS-${kusKey}`, tier };
                    }
                }
            }
    ...
    function wonderMatchCheck(input) {
        return input.match(/^(ICHBWMG|ICHBWPL)([0-9]{2})$/);
    }

    // Export internal mappings for compatibility wrapper reference
    window.COSY.entitlements.WONDER_DRAFT_MAPPING = WONDER_DRAFT_MAPPING;
    window.COSY.entitlements.KUS_DRAFT_MAPPING = KUS_DRAFT_MAPPING;
    window.COSY.entitlements.KUS_SPECIMEN_MAPPING = KUS_SPECIMEN_MAPPING;
    window.COSY.entitlements.HASH_DB = HASH_DB;
