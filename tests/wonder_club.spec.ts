...
test.describe('Wonder Club and Session Verification', () => {
  ...
  test('Do Insects Hide session page should load correctly', async ({ page }) => {
    ...
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Evacuate');
  });
  ...
  test('Is Bad Weather gods anger session page should load correctly', async ({ page }) => {
    ...
    await expect(page.locator('#vocabulary .vocab-card')).toHaveCount(10);
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Wrath');
  });
  ...
  test('Always Watched in a crowd session page should load correctly', async ({ page }) => {
    ...
    await expect(page.locator('#vocabulary .vocab-card')).toHaveCount(10);
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Spotlight');
  });

  test('Everyone is Copying Me session page should load correctly', async ({ page }) => {
    await page.goto('http://localhost:8080/apps/premium-events/clubs/wonder/sessions/i-couldnt-help-but-wonder/why-is-everyone-copying-me.html');
    await expect(page.locator('h1')).toContainText('Why Do I Feel Like Everyone Is Copying Me?');
    await expect(page.locator('#vocabulary .vocab-card')).toHaveCount(10);
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Mimic');
  });

  test('Whether Raindrops Select Where to Fall session page should load correctly', async ({ page }) => {
    await page.goto('http://localhost:8080/apps/premium-events/clubs/wonder/sessions/i-couldnt-help-but-wonder/whether-raindrops-select-where-to-fall.html');
    await expect(page.locator('h1')).toContainText('Do Raindrops Select the Place Where to Fall?');
    await expect(page.locator('#vocabulary .vocab-card')).toHaveCount(10);
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Speculate');
  });

  test('I Have No Time for It session page should load correctly', async ({ page }) => {
    await page.goto('http://localhost:8080/apps/premium-events/clubs/wonder/sessions/i-couldnt-help-but-wonder/i-have-no-time-for-it.html');
    await expect(page.locator('h1')).toContainText('I Have No Time for It');
    await expect(page.locator('#vocabulary .vocab-card')).toHaveCount(10);
    await expect(page.locator('#vocabulary .vocab-card').first().locator('.vocab-word')).toContainText('Bulletproof');
  });
});
