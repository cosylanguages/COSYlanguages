...
test.describe('Practice Interactive Deep Visual & Functional Test', () => {
    test('Check Practice Hub and Start Practice Session across viewports', async ({ page }) => {
        ...
        for (const vp of viewports) {
            ...
            await page.screenshot({ path: `verification/screenshots/practice_hub_${vp.name.replace(/\s+/g, '_')}.png`, fullPage: true });
            ...
            if (await startBtn.isVisible()) {
                ...
                await page.screenshot({ path: `verification/screenshots/practice_session_started_${vp.name.replace(/\s+/g, '_')}.png`, fullPage: true });
            }
        }
    });
    ...
    test('Check Cognitive Immersion across viewports and dark mode', async ({ page }) => {
        ...
        await page.waitForLoadState('domcontentloaded');
        await page.screenshot({ path: `verification/screenshots/cognitive_immersion_mobile_light.png`, fullPage: true });

        // Toggle dark mode if html attribute exists or body class
        await page.evaluate(() => document.documentElement.setAttribute('data-theme', 'dark'));
        await page.waitForTimeout(200);
        await page.screenshot({ path: `verification/screenshots/cognitive_immersion_mobile_dark.png`, fullPage: true });
    });
    ...
    test('Check practice exercise types (Flashcard, Vocab, Grammar, Reading, Listening, Writing)', async ({ page }) => {
        ...
        for (const t of types) {
            ...
            await page.screenshot({ path: `verification/screenshots/practice_type_${t}_mobile.png`, fullPage: true });
        }
    });
});
