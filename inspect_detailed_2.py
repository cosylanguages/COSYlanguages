...
for p in paths:
    ...
    for f in os.listdir(p):
        ...
        with open(filepath, 'r', encoding='utf-8') as file:
            ...
            # Level
            level_el = soup.find(string=lambda x: x and ('Level' in x or 'Niveau' in x or 'Уровень' in x))
            level = ""
            if level_el:
            ...
            # Theme
            theme_el = soup.find(string=lambda x: x and ('Theme' in x or 'Thème' in x or 'Тема' in x))
            theme = ""
            if theme_el:
...
for r in results:
    # check if vocab contains words that look inappropriate or mismatches
    # Let's see all files and their vocabularies to find mismatches
    print(f"{r['path']}/{r['file']} | {r['level']} | Heading: {r['heading'][:80]}")
    print(f"  Vocab: {', '.join(r['words'])}")
