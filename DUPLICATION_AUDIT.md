...
| **`games/`** | 0 files | *Directory does not exist at root.* | Removed / Migrated. | `games/index.html` acts as a directory hub pointing directly to external `COSYgames` endpoints. | **RESOLVED / ABSENT**<br>All minigames were successfully extracted to `COSYgames`. |
