...
test.describe('Long Reads (Literary Parlor) Club Verification', () => {
  test('Long Reads Hub loads with interactive Typewriter Ledger widget', async ({ page }) => {
    ...
    await page.screenshot({ path: 'project/tests/verification/screenshots/verification_long_reads_light.png' });

    // 5. Toggle dark mode and capture dark mode screenshot of the ledger
    await page.evaluate(() => document.documentElement.setAttribute('data-theme', 'dark'));
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'project/tests/verification/screenshots/verification_long_reads_dark.png' });
...
test.describe('If You Were Club Verification', () => {
  ...
  test('If You Were Deaf session page loads with Perspective Mirror box and Subjunctive anchor', async ({ page }) => {
    ...
    await page.screenshot({ path: 'project/tests/verification/screenshots/verification_if_you_were_light.png' });

    // 6. Toggle dark mode and capture dark mode screenshot of the perspective mirror box
    await page.evaluate(() => document.documentElement.setAttribute('data-theme', 'dark'));
    await page.waitForTimeout(500);
    await page.screenshot({ path: 'project/tests/verification/screenshots/verification_if_you_were_dark.png' });
