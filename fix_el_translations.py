...
themes_data = {
    "A0": {
        "desc": "Concrete, single-concept nouns and basic adjectives only",
        "themes": {
            "body_A0": "The body",
    ...
    "A1": {
        "desc": "Everyday adult life — familiar objects and situations",
        "themes": {
    ...
    "A2": {
        "desc": "Adult everyday contexts — work, health, money, relationships",
        "themes": {
    ...
    "B1": {
        "desc": "Broader adult society — work, values, health, environment",
        "themes": {
            "career_development_B1": "Career development & ambition",
            "word_life_balance_B1": "Work-life balance & wellbeing",
    ...
    "B2": {
        "desc": "Abstract adult themes — society, politics, economy, ethics",
        "themes": {
    ...
    "C1": {
        "desc": "Conceptual & professional — power, identity, philosophy of adult life",
        "themes": {
    ...
    "C2": {
        "desc": "Philosophical, rhetorical & meta-conceptual",
        "themes": {
...
new_translations = ""
for lvl, data in themes_data.items():
    desc_key = f"desc_{lvl.lower()}"
    new_translations += f'    {desc_key}: "{data["desc"]}",\n'
    for key, val in data["themes"].items():
        theme_key = f"theme_{key}"
        new_translations += f'    {theme_key}: "{val}",\n'

# el block is at the end
el_pattern = r'el: \{(.*)\}'
match = re.search(el_pattern, content, re.DOTALL)
if match:
